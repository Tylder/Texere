...
Framework-agnostic core tool types and definitions for Texere. This package defines the canonical
`CoreTool<I, O, State>` abstraction that can be used unchanged across Mastra and LangGraph.js via
thin framework-specific adapters.
...
- **Framework-agnostic**: Mastra and LangGraph.js receive identical tool definitions via mechanical
  adapters
...
- **CoreTool**: Framework-independent definition with `meta`, `handler`, input/output schemas, and
  effect support
- **ToolEnv**: Union type for Mastra and LangGraph environments; handlers branch on `env.framework`
  if needed
- **ToolEffect**: State updates and UI events that adapters translate into framework-native
  primitives
