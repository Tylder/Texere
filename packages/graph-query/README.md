...
> Typed graph query APIs for the Texere graph system.
...
1. Copy to `packages/<name>/`; replace `graph-query`,
   `Typed graph query APIs for the Texere graph system.`, `graph`.
...
- Lint: `pnpm nx run graph-query:lint`; Typecheck: `pnpm nx run graph-query:check-types`; Build
  emits d.ts to `dist/`.
