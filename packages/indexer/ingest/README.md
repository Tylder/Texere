...
> Indexer ingest pipeline: git snapshot orchestration, language indexers, extractors.
...
1. Copy to `packages/<name>/`; replace `indexer-ingest`,
   `Indexer ingest pipeline: git snapshot orchestration, language indexers, extractors.`, `indexer`.
...
- Lint: `pnpm nx run indexer-ingest:lint`; Typecheck: `pnpm nx run indexer-ingest:check-types`;
