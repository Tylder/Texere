...
> Indexer core: config loading, Neo4j/Qdrant clients, graph/vector helpers.
...
1. Copy to `packages/<name>/`; replace `indexer-core`,
   `Indexer core: config loading, Neo4j/Qdrant clients, graph/vector helpers.`, `indexer`.
...
- Lint: `pnpm nx run indexer-core:lint`; Typecheck: `pnpm nx run indexer-core:check-types`; Build
  emits d.ts to `dist/`.
