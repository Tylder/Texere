...
created: 2026-01-24
last_updated: 2026-01-24T11:28:52.151Z
area: knowledge
  ...
  sections:
    ...
    - title: 'TLDR'
      ...
      summary:
        ...
        lineage, keeps external sources structurally equivalent to internal code, and represents
...
Summary: Decisions and rationale decay because information fragments across docs, chats, and code. A
graph-based approach can fix this only if it avoids taxonomy sprawl, enforces epistemic lineage,
keeps external sources structurally equivalent to internal code, and represents multi-step plans
with dependencies and provenance.

**What:** Identify and document the core problems that a graph-based project memory must solve.

**Why:** Without a precise problem map, the system will either become an unqueryable dump or an
over-typed, brittle schema that cannot evolve.
...
- Problems and failure modes in capturing projects as nodes (desires/uncertainties/options/
  decisions/etc.)
...
This project will require a small, stable set of node categories to keep the graph queryable and
prevent semantic drift. These categories are not “document types.” They exist to ensure queries,
invariants, and provenance remain stable as the graph grows. Concrete schema details
...
A project should be documented as Nodes across the full lifecycle (ideation → problems → exploration
→ decisions → requirements → specifications → multi-step implementation plans → implementation →
verification), where every decision and commitment is stored, can be traced to evidence and
artifacts (including external sources and code), and remains queryable over time as the project
evolves.
...
- Verification/acceptance evidence (tests, checks, manual validation) is absent or not linked back
  to the commitments they validate
...
missing, too implicit, or overwritten by code reality. Even if requirements and decisions are
...
related_ideation:
  [IDEATION-EXPERIENCE-graph-knowledge-system, IDEATION-UNKNOWNS-graph-knowledge-system]
drives_to: REQ-graph-knowledge-system # Will be filled in after Requirements created
keywords:
  [graph, knowledge-system, provenance, evidence, lifecycle, plan, verification, query, as-of]
```
