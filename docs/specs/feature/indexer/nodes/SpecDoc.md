...
| `source`       | enum      | Required    | "colocated" \| "separate" \| "hosted" \| "generated"   |
| `content`      | string    | Optional    | Document content (first 5KB)                           |
| `embeddingId`  | string    | Optional    | Qdrant vector ID for similarity search                 |
| `generatedAt`  | timestamp | Optional    | When generated (only if source="generated")            |
| `generatedBy`  | string    | Optional    | LLM model name (only if source="generated")            |
| `generatedFor` | string    | Optional    | Target node ID (only if source="generated")            |
...
CREATE (doc:SpecDoc {
  ...
  generatedFor: "Symbol:UserService",
  embeddingId: "vec-abc456",
  createdAt: timestamp()
})
```
...
| ----------- | ---------------------------------------- | --------------------------------- |
| "colocated" | Markdown files in repo + code docstrings | File system read + AST extraction |
| "separate"  | Documentation in separate git repo       | Git clone + file read             |
| "hosted"    | Documentation as website                 | HTTP crawl + HTML parsing         |
| "generated" | Auto-generated by LLM                    | LLM prompt + code analysis        |
...
6. **Generated doc review**: "Show me all generated documentation"
7. **Documentation source analysis**: "Which docs are human-written vs. generated?"
